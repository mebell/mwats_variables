...
def re_plot(s_time, s_flux, s_error, name, m, x):
	...
	plt.subplots_adjust(hspace=0.5)
        ...
        #print avg_s
        print 'Mean Average Flux = '+str(np.mean(avg_s))
        print 'SD Average Flux = '+str(np.std(avg_s))
        print "Average M = "+str(avg_m)
...
for file in glob.glob("GLEAM*.txt"):
    ...
    avg_s.append(avg)
    std = np.std(fluxes)
    print 'Mean Flux = '+str(avg)
    print 'SD Flux = '+str(std)
    m = (std/avg)*100
    print "M = "+str(m) 
    print '\n'
